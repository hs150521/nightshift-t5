/**
 * @file t5_protocol.c
 * @brief T5-Link v1 codec: CRC-16/CCITT-FALSE, COBS, frame encode/decode.
 */
...
/* =========================================================================
 * CRC-16/CCITT-FALSE  (poly=0x1021, init=0xFFFF, refIn=0, refOut=0, xorOut=0)
 * ======================================================================= */
 ...
 * COBS encode
 *
 * Scans [src, src+len) and replaces 0x00 bytes with a code byte that
 * indicates the distance to the next zero (or end-of-block).  Runs of
 * non-zero bytes longer than 254 are split with a 0xFF code.
 * Returns encoded length (not including any 0x00 delimiter).
 * ======================================================================= */
                   ...
                   uint8_t *dst, size_t dst_max)
{
    ...
    if (out_idx >= dst_max) return 0;
    code_idx = out_idx++;   /* reserve first code slot */

    for (size_t i = 0; i < len; i++) {
        if (src[i] == 0x00) {
            /* Write code: distance from code_idx to here (inclusive) */
            dst[code_idx] = code;
            code    = 1;
            if (out_idx >= dst_max) return 0;
            code_idx = out_idx++;
                   ...
                   uint8_t *dst, size_t dst_max)
{
    ...
    while (i < len) {
        ...
        /* Insert implicit zero unless this was the last block or code==0xFF */
        if (code < 0xFF && i < len) {
            if (out_idx >= dst_max) return 0;
            dst[out_idx++] = 0x00;
        }
 ...
 * Frame encode
 *
 * Wire format:  COBS( raw_frame_with_crc ) + 0x00
 * raw_frame  :  Magic(2) + Version(1) + Flags(1) + Seq(2LE) + Cmd(2LE)
 *               + PayloadLen(2LE) + Payload(N) + CRC16(2LE)
 * CRC is computed over Magic..Payload (the first 10+N bytes).
 * ======================================================================= */
                       ...
                       uint8_t *out, size_t out_max)
{
    ...
    /* Header */
    raw[0] = T5_MAGIC_BYTE0;
    raw[1] = T5_MAGIC_BYTE1;
    raw[2] = T5_PROTO_VERSION;
    raw[3] = frame->flags;
    WRITE_U16_LE(raw + 4,  frame->seq);
    WRITE_U16_LE(raw + 6,  frame->cmd);
    WRITE_U16_LE(raw + 8,  frame->payload_len);

    /* Payload */
    if (frame->payload_len > 0) {
        memcpy(raw + 10, frame->payload, frame->payload_len);
    }

    /* CRC16 over Magic..Payload */
    uint16_t crc = crc16_ccitt_false(raw, 10 + frame->payload_len);
    WRITE_U16_LE(raw + 10 + frame->payload_len, crc);

    /* COBS encode */
    size_t cobs_len = cobs_encode(raw, raw_len, out, out_max - 1);
...
/* =========================================================================
 * Frame decode
 * Input: COBS bytes (everything before the 0x00 delimiter).
 * ======================================================================= */
                    ...
                    t5_frame_t *frame)
{
    ...
    if (raw_len < 12) return -2;   /* too short for minimum frame */

    /* Validate magic */
    if (raw[0] != T5_MAGIC_BYTE0 || raw[1] != T5_MAGIC_BYTE1) return -3;

    /* Validate version */
    if (raw[2] != T5_PROTO_VERSION) return -4;

    /* Extract fields (all LE) */
    uint8_t  flags       = raw[3];
    uint16_t seq         = READ_U16_LE(raw + 4);
    uint16_t cmd         = READ_U16_LE(raw + 6);
